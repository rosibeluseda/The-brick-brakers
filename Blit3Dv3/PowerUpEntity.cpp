...
extern Sprite* catchBallSprites[];
extern Sprite* largePaddleSprites[];
...
void PowerUpEntity::Update(float seconds)
{
	...
	while(frameTimer >= timeBetweenFrames)
	{
		...
		currentFrameNum++;
		if(currentFrameNum > 13) //loop the animation frames
			currentFrameNum = 0;
...
PowerUpEntity * MakePowerUp(PowerUpType type, float xpos, float ypos)
{
	...
	case PowerUpType::LARGERPADDLE:
		for (int i = 0; i < 14; ++i)
			powerUpEntity->spriteList[i] = largePaddleSprites[i];
		powerUpEntity->sprite = powerUpEntity->spriteList[0];
		break;
	case PowerUpType::CATCH:
	default:
		for (int i = 0; i < 14; ++i)
			powerUpEntity->spriteList[i] = catchBallSprites[i];
		powerUpEntity->sprite = powerUpEntity->spriteList[0];
		break;
	}
	...
	boxShape.SetAsBox(48.0f / (2.f*PTM_RATIO), 48.0f / (2.f*PTM_RATIO));
